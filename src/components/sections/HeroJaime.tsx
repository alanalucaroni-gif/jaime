...
export function HeroJaime() {
  ...
  useGSAP(() => {
    ...
    scroll.to("[data-product]", { yPercent: -6, scale: .74, rotate: 180, transformOrigin: "50% 55%", duration: 1.05, ease: "power2.inOut" }, 0)
      .to("[data-slam='0']", { xPercent: -18 }, 0)
      .to("[data-slam='1']", { xPercent: 15 }, 0)
      .to("[data-slam='2']", { xPercent: -10 }, 0)
      .to("[data-hero-fade]", { opacity: .18, y: -55 }, .35)
      .fromTo("[data-falling-fruit]", { opacity: 0, scale: .25, y: 0, x: 0 }, { opacity: 1, scale: i => [1.05,.8,1.18,.86,1][i], y: () => innerHeight * 1.04, x: i => [-330,-155,5,175,340][i], rotate: i => [-320,260,-210,360,290][i], duration: 1.25, stagger: .07, ease: "power1.in" }, 1.02)
      .fromTo("[data-falling-cookie]", { opacity: 0, scale: .2, y: 0, x: 0 }, { opacity: 1, scale: i => [.92,1.16,.82][i], y: () => innerHeight * .98, x: i => [-230,70,275][i], rotate: i => [280,-340,230][i], duration: 1.18, stagger: .11, ease: "power1.in" }, 1.12);
  }, { scope: ref });
