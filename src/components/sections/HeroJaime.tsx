...
export function HeroJaime() {
  ...
  useGSAP(() => {
    ...
    scroll.to("[data-product]", { yPercent: -8, scale: .56, rotate: 180, transformOrigin: "50% 55%", duration: 1.05, ease: "power2.inOut" }, 0)
      .to("[data-slam='0']", { xPercent: -18 }, 0)
      .to("[data-slam='1']", { xPercent: 15 }, 0)
      .to("[data-slam='2']", { xPercent: -10 }, 0)
      .to("[data-hero-fade]", { opacity: .18, y: -55 }, .35)
      .fromTo("[data-falling-fruit]", { opacity: 0, scale: .25, y: 0, x: 0 }, { opacity: 1, scale: i => [.86,.62,1,.7,.8][i], y: () => innerHeight * 1.05, x: i => [-300,-135,10,155,310][i], rotate: i => [-320,260,-210,360,290][i], duration: 1.25, stagger: .07, ease: "power1.in" }, 1.02);
  }, { scope: ref });
